...
def get_conditional_EI_expectation_and_variance(df, energy_dict):
    ...
    energy_intensities = np.array([energy_dict.get(mode, 0) for mode in df.index]) # this will place each intensity in the same order as it appears in the confusion matrix. # changed ///
