...
def get_EI_moments_for_trip(mode,os,android_EI_moments,ios_EI_moments):
    ...
    if os == "android":
        if mode in android_EI_moments["mean(EI)"]: #///////
            ...
            if mode == 'Gas Car, with others':
                mean_EI = android_EI_moments["mean(EI)"].get('Gas Car, drove alone', 0)
                var_EI = android_EI_moments["variance(EI)"].get('Gas Car, drove alone', 0)
            elif mode == 'Gas Car, drove alone':
                mean_EI = android_EI_moments["mean(EI)"].get('Gas Car, with others', 0)
                var_EI = android_EI_moments["variance(EI)"].get('Gas Car, with others', 0)
            else:
            #     mean_EI = android_EI_moments["mean(EI)"][mode]
            #     var_EI = android_EI_moments["variance(EI)"][mode]
                mean_EI = 0
                var_EI = 0
    elif os == "ios":
        # mean_EI = ios_EI_moments["mean(EI)"][mode]
        # var_EI = ios_EI_moments["variance(EI)"][mode]
        if mode in ios_EI_moments["mean(EI)"]: #///////
            mean_EI = ios_EI_moments["mean(EI)"][mode]
            var_EI = ios_EI_moments["variance(EI)"][mode]
        else:
            if mode == 'Gas Car, with others':
                mean_EI = ios_EI_moments["mean(EI)"].get('Gas Car, drove alone', 0)
                var_EI = ios_EI_moments["variance(EI)"].get('Gas Car, drove alone', 0)
            elif mode == 'Gas Car, drove alone':
                mean_EI = ios_EI_moments["mean(EI)"].get('Gas Car, with others', 0)
                var_EI = ios_EI_moments["variance(EI)"].get('Gas Car, with others', 0)
            else:
                # mean_EI = ios_EI_moments["mean(EI)"][mode]
                # var_EI = ios_EI_moments["variance(EI)"][mode]
                mean_EI = 0
...
def get_user_labeled_EC_for_one_trip(ct,unit_dist_MCS_df,energy_dict):
    ...
    if mode == np.nan or type(mode) == float: 
        ...
        EI = energy_dict[MODE_MAPPING_DICT[mode]] #/// initial code
        # EI = energy_dict.get(MODE_MAPPING_DICT[mode], 0) # //// changed, seems only air will happened, but we drop all the air trips in the whole dataset now.
...
def compute_aggregate_variance_by_primary_mode(df, os_EI_moments_map, unit_dist_MCS_df):
    ...
    for os in df.os.unique():
        ...
        for primary_mode in trips_grouped_by_primary_mode.index:
            ...
            if primary_mode in os_EI_moments_map[os]["mean(EI)"]: #///////
                mean_EI = os_EI_moments_map[os]["mean(EI)"][primary_mode] 
                var_EI = os_EI_moments_map[os]["variance(EI)"][primary_mode]
            else:
                if primary_mode == 'Gas Car, with others':
                    mean_EI = os_EI_moments_map[os]["mean(EI)"].get('Gas Car, drove alone', 0)
                    var_EI = os_EI_moments_map[os]["variance(EI)"].get('Gas Car, drove alone', 0)
                elif primary_mode == 'Gas Car, drove alone':
                    mean_EI = os_EI_moments_map[os]["mean(EI)"].get('Gas Car, with others', 0)
                    var_EI = os_EI_moments_map[os]["variance(EI)"].get('Gas Car, with others', 0)
                else:
                    # mean_EI = os_EI_moments_map[os]["mean(EI)"][primary_mode] 
                    # var_EI = os_EI_moments_map[os]["variance(EI)"][primary_mode]
                    mean_EI = 0
