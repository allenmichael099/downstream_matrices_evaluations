...
from sklearn.model_selection import KFold, ParameterGrid, ParameterSampler, train_test_split
...
def one_cross_val_predict(model,
                      model_params=None,
                      user_df=None,
                      k=1,
                      random_state=42,
                      min_samples=False):
    """ Rendomly seperate dataset to training, test and validation dataset
        Conducts 1-fold cross-validation between training and test dataset.
    ...
    # Rendomly seperate dataset to training_model dataset(80%) and validation dataset(20%)
    # training_model dataset including training dataset(60%) and test dataset(20%)
    # Conducts 1-fold cross-validation between training and test dataset.
    
    # user_df['mode_confirm'] = user_df['mode_confirm'].replace(confusion_matrix_handling.MODE_MAPPING_DICT)
    ...
    # Separate dataset to  "train" 0.6, "test" 0.2, "validation"0.2 dataset.
    # Steps:
    # 1: clean all the data 
    # 2: extract the rows with only one member from all the data, then split the dataset into validation and temp sets 
    # 3: extract the rows with only one member from the temp set, then split the dataset into train and test sets 
    # 4: combine the single_dfs
    # 5: randomly assign the rows from combined_single_df to train_trips, test_trips, or validation_data


    # Identify the rows with only one member
    single_classes = list(user_df['mode_true'].value_counts().loc[lambda x: x == 1].index)

    # Filter the DataFrame to include only the single classes
    single_df = user_df[user_df['mode_true'].isin(single_classes)]

    # Remove the single classes from the original DataFrame
    modified_df = user_df[~user_df['mode_true'].isin(single_classes)]

    # Split the modified dataset into train and temp sets (80% train + temp, 20% validation)
    train_temp_df, validation_data = train_test_split(modified_df, test_size=0.2, random_state=49, stratify = modified_df.mode_true)


    # Identify the class with only one member
    single_classes_1 = list(train_temp_df['mode_true'].value_counts().loc[lambda x: x == 1].index)

    # Filter the DataFrame to include only the single classes
    single_df_1 = train_temp_df[train_temp_df['mode_true'].isin(single_classes_1)]

    # Remove the single classes from the original DataFrame
    modified_df_1 = train_temp_df[~train_temp_df['mode_true'].isin(single_classes_1)]

    combined_single_df = pd.concat([single_df, single_df_1], axis=0)

    # Further split the temp set into train and test sets (60% train, 20% test)
    train_trips, test_trips = train_test_split(modified_df_1, test_size=0.25, random_state=50, stratify = modified_df_1.mode_true)

    # Randomly assign the rows from combined_single_df to train_trips, test_trips, or validation_data
    for _, row in combined_single_df.iterrows():
        random_set = np.random.choice(["train", "test", "validation"], p=[0.6, 0.2, 0.2])
        if random_set == "train":
            train_trips = train_trips.append(row)
        elif random_set == "test":
            test_trips = test_trips.append(row)
        else:
            validation_data = validation_data.append(row)

    logging.debug(f'num trips {len(user_df)}')
    logging.debug(f'num training trips {len(train_trips)}')
    logging.debug(f'num test trips {len(test_trips)}')
    logging.debug(f'num validation trips {len(validation_data)}')

    if not min_samples and (len(train_trips) < 1 or len(test_trips) < 1 or len(validation_data) < 1):
        logging.info(
            'At least 1 trip is needed for each dataset, user only had {}.'
            .format(len(train_trips) + '_' + len(test_trips) + '_' + len(validation_data)))
        return
    ...
    logging.info("About to generate predictions for the model %s" % model)
    # generate predictions for test_trips
    ...
    next_replaced_pred = pred_df['replaced_pred']
     ...
     # generate predictions for validation_trips
    ...
    # handle case where users input partial labels (e.g. some users
    # never input replaced-mode)
    if 'mode_true' in test_trips.columns:
        mode_initial = test_trips['mode_initial'].to_list()
        mode_true = test_trips['mode_true'].to_list()
    else:
        mode_initial = list(np.full(len(test_trips), np.nan))
        mode_true = list(np.full(len(test_trips), np.nan))
    if 'purpose_true' in test_trips.columns:
        purpose_true = list(test_trips['purpose_true'].to_list())
    else:
        purpose_true = np.full(len(test_trips), np.nan)
    if 'replaced_true' in test_trips.columns:
        replaced_true = list(test_trips['replaced_true'].to_list())
    else:
        replaced_true = list(np.full(len(test_trips), np.nan))
    ...
    return {
        ...
        'train_trips_len': len(train_trips)
    }


def k_fold_cross_val_predict(model,
                      model_params=None,
                      user_df=None,
                      k=1,
                      random_state=42,
    ...
    """ Stratified seperate dataset to temp dataset(80% of the whole dataset) and validation dataset((20% of the whole dataset) based on the true trip mode
        ...
        Returns: 
            dict containing lists of 'idx', 'mode_initial', 'mode_true', 'purpose_true', 'replaced_true', 'mode_pred', 'purpose_pred', 
            'replaced_pred', 'distance', 'duration', 'validation_data', 'train_trips_len', 'fold_number_list' for the whole dataset.
            "validation_data" contains the validation dataset.
    ...
    # Steps:
    # 1: clean all the data 
    # 2: extract the rows with one count mode from all the data, then split the dataset into validation and temp sets 
    # 3: randomly assign the rows from single_count_df to temp or validation_data with p=[0.8, 0.2]

    # Identify the rows with only one count
    single_count = list(user_df['mode_true'].value_counts().loc[lambda x: x == 1].index)

    # Filter the DataFrame to include only the single count
    single_count_df = user_df[user_df['mode_true'].isin(single_count)]
    ...
    return {
        ...
        'distance': distance,
        'duration': duration,
        'validation_data': validation_data, # contains the validation dataset
        'train_trips_len': len(train_trips),
        'fold_number_list': fold_number_List
    }
...
def cv_for_all_users(model,
    ...
    # returns a combined dataframe of validation dataset
    validation_dfs = []
    ...
    for user in uuid_list:
        ...
        try:
            ...
            # Separating the dataset to 60%_training, 20%_testing, 20%_validation
            # results = one_cross_val_predict(model,
            #                             model_params,
            #                             user_df=expanded_trip_df_map[user],
            #                             k=1,
            #                             random_state=random_state,
            ...
            # Separating the dataset to 80%_temp, 20%_validation. 
            # For the 80%_temp, we do the k-fold cross validation get the prediction of 80%_temp. After k-fold cross validation, we get the prediction of 80%_temp.
            # Then, we  aim this 80%_temp as the testing dataset and return it in the "results"
            # The "results" also conatins validation dataset which is in the "validation_data". We use the 80%_temp to training the model and predict the validation dataset.
            # All in all, "results" contains testing dataset and validation dataset.
            results = k_fold_cross_val_predict(model,
                                        ...
                                        random_state=random_state,
                                        min_samples=min_samples)
            validation_data = results.pop('validation_data', None)  # pop the validation dataset, so the rest of results will be testing dataset.
            train_trips_len += results.get('train_trips_len') 
        ...
        try:
            ...
            validation_data_sets = pd.DataFrame(data=validation_data)
        ...
        validation_data_sets['user_id'] = user
        validation_dfs += [validation_data_sets]
    ...
    validation_all = pd.concat(validation_dfs, ignore_index=True)
    validation_all['dataset'] = 'validation_dataset'
    combined_df = pd.concat([cross_val_all, validation_all], ignore_index=True)
    
    logging.debug(f'total num training trips {train_trips_len}')
...
def get_clf_metrics(trip_df,
    ...
    # Wen comments: The below code is checking whether the predicted value appeared in the initial dataset.
    # We don't need it for test trips, because test trips may not have the purpose/mode/replaced in training trips.
    ...
    duration = labeled_predicted_df['duration'].astype(float)

    user_id = labeled_predicted_df['user_id']
