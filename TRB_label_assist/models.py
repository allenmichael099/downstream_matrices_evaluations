...
class SetupMixin(metaclass=ABCMeta):
    ...
    def _clean_data(self, df):
        ...
        for category in ['mode_true', 'purpose_true', 'replaced_true']:
            ...
            if (category == 'mode_true') and ('mode_true' in df.columns):
                df['mode_initial'] = df['mode_true'] # Saving the initial mode name, for connecting with the evaluation methods.
                df['mode_true'] = df['mode_true'].replace(confusion_matrix_handling.MODE_MAPPING_DICT)
                print('Dropping user labeled AIR trips in the data set.')
                df = df.drop(df[df.mode_true == 'Air'].index) # drop all trips which mode is 'Air'///
...
class EnsembleClassifier(TripClassifier, metaclass=ABCMeta):
    ...
    def preprocess_data(self, df):
        if df.purpose_true.isna().any():
            num_nan = df.purpose_true.value_counts(
                dropna=False).loc[np.nan]
            logging.info(
                f'dropping {num_nan}/{len(df)} trips that are missing purpose labels'
            )
            df = df.dropna(
                subset=['purpose_true']).reset_index(drop=True)
        if len(df) == 0:
    ...
    def fit(self, train_df):
        ...
        if self.loc_feature == 'cluster':
            ...
            # TODO: move below code into a reusable function - Wen comments: done
            ...
            # self.train_df['mode_true'] = self.train_df['mode_true'].replace(confusion_matrix_handling.MODE_MAPPING_DICT)
